...
export default function SessionPage() {
  ...
  const getGameDisplayImageSafe = (session: Session) => {
    const game = getGameForSession(session);
    return game ? getGameDisplayImage(game) : { type: "avatar" as const, value: "🎲" };
  };
  ...
  // Helper component for rendering game display
  const GameDisplay = ({ session, size = "default" }: { session: Session; size?: "small" | "default" }) => {
    const game = getGameForSession(session);
    const displayImage = getGameDisplayImageSafe(session);

    const imageSize = size === "small" ? "w-8 h-8" : "w-12 h-12";
    const textSize = size === "small" ? "text-xl" : "text-3xl";

    return (
        <div className="flex items-center space-x-3">
          {displayImage.type === 'thumbnail' ? (
              <img
              ...
              <div className={textSize}>{displayImage.value}</div>
          )}
          <div>
            <h3 className={`font-semibold text-neutral-900 ${size === "small" ? "text-lg" : "text-xl"}`}>
              ...
              <div className="bg-white rounded-lg border-2 border-green-500 p-6">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <GameDisplay session={currentSession} />

                    <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-8 text-sm text-neutral-600 mt-3">
                    ...
                    {currentSession.notes && currentSession.notes.length > 0 && (
                        <div className="flex items-center space-x-1 text-sm text-neutral-500 mt-2">
                          <IconNote size={16} />
                          <span>
                ...
                <button
                    ...
                    className="mt-6 bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition-colors flex justify-center items-center space-x-2 w-full"
                >
                ...
                {previousSessions.map((session) => {
                  ...
                  return (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            <GameDisplay session={session} size="small" />

                            <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-6 text-sm text-neutral-600 mt-3">
                              <div className="flex items-center space-x-1">
                                <IconUsers size={16} />
